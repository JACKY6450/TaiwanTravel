  ...
  <div class="kaosiungInfo">
    ...
    <header class="header">
      <div class="container text-center">
        <h1 class="text-white" style="margin-bottom: 95px">南投旅遊資訊</h1>
  ...
  methods: {
    ...
    setPopular() {
      ...
      this.locations.forEach((item) => {
        if(['魚池鄉', '信義鄉', '水里鄉', '鹿谷鄉'].includes(item)){
          popularSet.push({
            Zone: item,
          });
        }
    ...
    recall(){
      ...
      this.$http.get(apiUrl).then((response) => {
        ...
        let tempData = data.filter((item) => {
          if(item.Region === '南投縣') return item
        })
...
<style scoped> 
  .header {
    background: url("../../assets/pic/bannerNantou.jpg");
