  ...
  <div class="kaosiungInfo">
    ...
    <header class="header">
      <div class="container text-center">
        <h1 class="text-white" style="margin-bottom: 95px">新北旅遊資訊</h1>
  ...
  methods: {
    ...
    setPopular() {
      ...
      this.locations.forEach((item) => {
        if(['淡水區', '金山區', '石門區', '三峽區'].includes(item)){
          popularSet.push({
            Zone: item,
          });
        }
    ...
    recall(){
      ...
      this.$http.get(apiUrl).then((response) => {
        ...
        let tempData = data.filter((item) => {
          if(item.Region === '新北市') return item
        })
...
<style scoped> 
  .header {
    background: url("../../assets/pic/bannerNewTaipei.jpg");
